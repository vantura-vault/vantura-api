...
export const vaultController = {
  ...
  // POST /api/vault/competitors/refresh-all
  async refreshAllCompetitors(req: Request, res: Response): Promise<void> {
    try {
      ...
      const data = await vaultService.refreshAllCompetitors(companyId);
      ...
      console.error('Refresh all competitors error:', error);
      res.status(500).json({
        success: false,
        error: error instanceof Error ? error.message : 'Failed to refresh competitors',
      });
    }
  },
